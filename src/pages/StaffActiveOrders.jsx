    ...
    <Popover>
      <PopoverTrigger asChild>
        <button
          ...
          className="flex items-center justify-center w-5 h-5 rounded-full bg-amber-100 hover:bg-amber-200 transition-colors flex-shrink-0"
        >
...
function getAlertReasons(order, isSuspicious, avg) {
  const reasons = [];
  if (order.status === 'created' && isAboveThreshold(order.amount, order.currency))
    reasons.push(`Amount ≥ 25,000 USD equivalent — regulatory reporting threshold`);
  if (isSuspicious && avg)
    reasons.push(
      `Amount is ${(parseFloat(order.amount) / avg).toFixed(1)}x above this client's historical average (avg: ${order.currency} ${avg.toLocaleString('en-US', { maximumFractionDigits: 0 })})`
    );
  return reasons;
}
...
function OrderRow({ order, selectedIds, onSelect, onOpen, isSuspicious, avg, clientName }) {
  ...
  return (
    <TableRow
      className={`border-slate-200 hover:bg-blue-50/50 cursor-pointer transition-colors ${hasAlert ? 'bg-amber-50/30' : ''}`}
    >
      <TableCell className="w-10 py-2 pl-4" onClick={(e) => e.stopPropagation()}>
        <Checkbox
          checked={selectedIds.has(order.orderId)}
          onCheckedChange={(checked) => onSelect(order.orderId, checked)}
        />
      </TableCell>
      ...
      <TableCell className="w-20 py-2 text-center cursor-pointer" onClick={() => onOpen(order)}>
        <Badge className={`text-xs ${order.invocieReceived ? 'bg-emerald-600' : 'bg-slate-300 text-slate-600'}`}>
          {order.invocieReceived ? 'Y' : 'N'}
        </Badge>
      </TableCell>
      <TableCell className="w-20 py-2 text-center cursor-pointer" onClick={() => onOpen(order)}>
        <Badge className={`text-xs ${order.paymentProof ? 'bg-emerald-600' : 'bg-slate-300 text-slate-600'}`}>
          {order.paymentProof ? 'Y' : 'N'}
        </Badge>
      </TableCell>
      <TableCell className="w-20 py-2 text-center cursor-pointer" onClick={() => onOpen(order)}>
        <Badge className={`text-xs ${order.nonMandiriExecution ? 'bg-orange-500' : 'bg-slate-300 text-slate-600'}`}>
          {order.nonMandiriExecution ? 'Y' : 'N'}
        </Badge>
      </TableCell>
      <TableCell
        className="min-w-[100px] text-slate-600 text-xs font-mono py-2 cursor-pointer"
...
export default function StaffActiveOrders() {
  ...
  // Prefetch reference data for drawer — prevents race condition on first open
  useQuery({ queryKey: ['payeer-accounts'], queryFn: () => apiClient.getPayeerAccounts() });
  useQuery({ queryKey: ['currencies'], queryFn: () => apiClient.getCurrencies() });
  ...
  const clientGroups = useMemo(() => {
    ...
    filteredOrders.forEach((o) => {
      ...
      if (!map[key])
        map[key] = { client_id: o.clientId, client_name: clientsMap[o.clientId]?.name || o.clientId, orders: [] };
      ...
      <Table>
        <TableHeader>
          <TableRow className="bg-slate-50/70 hover:bg-slate-50/70 border-slate-100">
            ...
            {showClientColumn && (
              <TableHead className="min-w-[160px] text-slate-500 font-semibold text-xs">{t('clientLabel')}</TableHead>
            )}
  ...
  return (
    <div className="min-h-screen bg-slate-50">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <Badge className="bg-[#f5a623] text-white">
                  {activeOrders.length} {t('ordersLabel')}
                </Badge>
              </div>
            </div>
              ...
              {selectedIds.size > 0 && (
                <>
                  <span className="text-white/70 text-sm">
                    {selectedIds.size} {t('selectedLabel')}
                  </span>
                  <Button
      ...
      <main className="max-w-full mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="flex items-center gap-4 mb-5 flex-wrap">
          <div className="relative w-72">
            ...
            <Input
              placeholder={t('searchActiveOrdersPlaceholder')}
              value={search}
          ...
          <button
            onClick={() =>
              setSelectedIds(new Set(filteredOrders.filter((o) => o.status === 'on_execution').map((o) => o.orderId)))
            }
            ...
            {groupByClient ? (
              ...
              clientGroups.length === 0 ? (
                ...
                clientGroups.map(({ client_id, client_name, orders: clientOrders }) => {
                  ...
                  const totalByCur = ['USD', 'EUR', 'CNY']
                    .map((cur) => {
                      const t = clientOrders
                        .filter((o) => o.currency === cur)
                        .reduce((s, o) => s + (parseFloat(o.amount) || 0), 0);
                      return t > 0 ? `${cur} ${t.toLocaleString()}` : null;
                    })
                    .filter(Boolean);
                  const hasAlert = clientOrders.some(
                    (o) =>
                      suspiciousOrderIds.has(o.orderId) ||
                      (o.status === 'created' && isAboveThreshold(o.amount, o.currency))
                  );
                  const statusesPresent = ACTIVE_STATUSES.filter((s) => clientOrders.some((o) => o.status === s));
                  return (
                    <div
                      key={client_id}
                      className={`bg-white rounded-xl shadow-sm overflow-hidden border-2 ${hasAlert ? 'border-amber-300' : 'border-slate-200'}`}
                    >
                      {/* Client header */}
                      <button
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div
                            className={`w-9 h-9 rounded-xl flex items-center justify-center text-sm font-bold text-white shadow-sm ${hasAlert ? 'bg-amber-500' : 'bg-[#1e3a5f]'}`}
                          >
                          ...
                          <Badge className={`text-xs ${hasAlert ? 'bg-amber-500' : 'bg-[#1e3a5f]'} text-white`}>
                            {clientOrders.length} {t('clientOrdersBadge')}
                          </Badge>
                        </div>
                        <div className="flex items-center gap-4">
                          <div className="flex gap-2 text-xs text-slate-500">
                            {totalByCur.map((t) => (
                              <span key={t} className="bg-white border border-slate-200 rounded px-2 py-0.5">
                                {t}
                              </span>
                            ))}
                          </div>
                          <div className="flex gap-1.5 flex-wrap">
                            {statusesPresent.map((s) => {
                              ...
                              return (
                                <span
                                  key={s}
                                  className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${c.color} text-white`}
                                >
                                  {c.label} {cnt}
                                </span>
                              );
                                  ...
                                  <Table>
                                    <TableHeader>
                                      <TableRow className="bg-slate-50/50 hover:bg-slate-50/50 border-slate-100">
                                        <TableHead className="w-10 pl-4" />
                                        <TableHead className="w-36 text-slate-500 font-semibold text-xs">
                                          {t('orderId')}
                                        </TableHead>
                                        <TableHead className="w-44 text-slate-500 font-semibold text-xs">
                                          {t('amountLabel')}
                                        </TableHead>
                                        <TableHead className="w-10" />
                                        <TableHead className="w-20 text-center text-slate-500 font-semibold text-xs">
                                          {t('invRcv')}
                                        </TableHead>
                                        <TableHead className="w-20 text-center text-slate-500 font-semibold text-xs">
                                          {t('proof')}
                                        ...
                                        <TableHead className="text-slate-500 font-semibold text-xs pr-4">
                                          {t('invNum')}
                                        </TableHead>
                                      </TableRow>
                                    </TableHeader>
                                    <TableBody>
                                      {statusOrders.map((order) => (
                                        <TableRow
                                          key={order.orderId}
                                          className={`border-slate-100 hover:bg-blue-50/50 cursor-pointer transition-colors ${(order.status === 'created' && isAboveThreshold(order.amount, order.currency)) || suspiciousOrderIds.has(order.orderId) ? 'bg-amber-50/30' : ''}`}
                                          onClick={() => openDrawer(order)}
                                        >
                                          <TableCell className="w-10 py-2 pl-4" onClick={(e) => e.stopPropagation()}>
                                            <Checkbox
                                              checked={selectedIds.has(order.orderId)}
                                              onCheckedChange={(c) => handleSelectOne(order.orderId, c)}
                                            />
                                          </TableCell>
                                          <TableCell className="w-36 font-mono text-xs text-[#1e3a5f] py-2">
                                            <div className="flex items-center gap-1">
                                              {order.orderId}
                                              {order.invocieRequired && (
                                          ...
                                          <TableCell className="w-44 py-2">
                                            <span
                                              className={`font-medium text-xs ${(order.status === 'created' && isAboveThreshold(order.amount, order.currency)) || suspiciousOrderIds.has(order.orderId) ? 'text-amber-700' : 'text-[#1e3a5f]'}`}
                                            >
                                              {parseFloat(order.amount || 0).toLocaleString()} {order.currency}
                                            </span>
                                          </TableCell>
                                          <TableCell className="w-10 py-2">
                                            <AlertIcon
                                              reasons={getAlertReasons(
                                                order,
                                                suspiciousOrderIds.has(order.orderId),
                                                computeClientAverage(orders, order.clientId, order.orderId)
                                              )}
                                            />
                                          </TableCell>
                                          <TableCell className="w-20 text-center py-2">
                                            <Badge
                                              className={`text-xs ${order.invocieReceived ? 'bg-emerald-600' : 'bg-slate-300 text-slate-600'}`}
                                            >
                                              {order.invocieReceived ? 'Y' : 'N'}
                                            </Badge>
                                          </TableCell>
                                          <TableCell className="w-20 text-center py-2">
                                            <Badge
                                              className={`text-xs ${order.paymentProof ? 'bg-emerald-600' : 'bg-slate-300 text-slate-600'}`}
                                            >
                                              {order.paymentProof ? 'Y' : 'N'}
                                            </Badge>
                                          </TableCell>
                                          <TableCell className="w-20 text-center py-2">
                                            <Badge
                                              className={`text-xs ${order.nonMandiriExecution ? 'bg-orange-500' : 'bg-slate-300 text-slate-600'}`}
                                            >
                                              {order.nonMandiriExecution ? 'Y' : 'N'}
                                            </Badge>
                                          </TableCell>
                                          <TableCell className="text-slate-600 text-xs font-mono py-2 pr-4">
                                            {order.invoiceNumber || '-'}
                                          </TableCell>
                                        </TableRow>
                                      ))}
      ...
      <AlertDialog
        open={!!suspiciousAlertOrder}
        onOpenChange={(open) => {
          if (!open) setSuspiciousAlertOrder(null);
        }}
      >
        <AlertDialogContent className="bg-white border-orange-300 border-2 max-w-md">
            ...
            <AlertDialogDescription asChild>
              <div className="space-y-2 text-sm">
                <p className="text-slate-700">
                  <strong>
                    {clientsMap[suspiciousAlertOrder?.order?.clientId]?.name || suspiciousAlertOrder?.order?.clientId}
                  </strong>{' '}
                  {t('unusualTransactionDesc')}
                </p>
                <div className="bg-orange-50 border border-orange-200 rounded-lg p-3 space-y-1">
                  <div className="flex justify-between text-xs">
                    <span className="text-slate-600">{t('thisOrder')}</span>
                    <span className="font-bold text-orange-700">
                      {suspiciousAlertOrder?.order?.currency}{' '}
                      {parseFloat(suspiciousAlertOrder?.order?.amount || 0).toLocaleString('en-US')}
                    </span>
                  </div>
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="font-semibold text-slate-700">
                      {suspiciousAlertOrder?.order?.currency}{' '}
                      {suspiciousAlertOrder?.avg?.toLocaleString('en-US', { maximumFractionDigits: 0 })}
                    </span>
                  </div>
                  <div className="flex justify-between text-xs border-t border-orange-200 pt-1 mt-1">
                    ...
                    <span className="font-bold text-red-600">
                      {suspiciousAlertOrder?.ratio?.toFixed(1)}
                      {t('xAboveAverage')}
                    </span>
                  </div>
                </div>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setSuspiciousAlertOrder(null)} className="border-slate-300">
              {t('cancelLabel')}
            </AlertDialogCancel>
            <AlertDialogAction
              onClick={() => {
                setSelectedOrder(suspiciousAlertOrder.order);
                setDrawerOpen(true);
                setSuspiciousAlertOrder(null);
              }}
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent className="bg-slate-800 border-slate-700 text-white">
          <AlertDialogHeader>
            <AlertDialogTitle>
              {t('deleteLabel')} {selectedIds.size} {t('deleteOrdersTitle')}
            </AlertDialogTitle>
