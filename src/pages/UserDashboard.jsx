        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
          <Card className="bg-white border-slate-200">
            <CardContent className="pt-6">
              <div className="text-3xl font-bold text-[#1e3a5f]">{ordersLoading ? '—' : stats.total}</div>
              <div className="text-sm text-slate-600 mt-1">{stats.labels.total}</div>
            </CardContent>
          </Card>
          <Card className="bg-white border-slate-200">
            <CardContent className="pt-6">
              <div className="text-3xl font-bold text-[#f5a623]">{ordersLoading ? '—' : stats.current}</div>
              <div className="text-sm text-slate-600 mt-1">{stats.labels.current}</div>
            </CardContent>
          </Card>
          <Card className="bg-white border-slate-200">
            <CardContent className="pt-6">
              <div className="text-3xl font-bold text-emerald-600">{ordersLoading ? '—' : stats.executed}</div>
              <div className="text-sm text-slate-600 mt-1">{stats.labels.executed}</div>
            </CardContent>
          </Card>
          <Card className="bg-white border-slate-200">
            <CardContent className="pt-6">
              <div className="text-3xl font-bold text-red-600">{ordersLoading ? '—' : stats.cancelled}</div>
              <div className="text-sm text-slate-600 mt-1">{stats.labels.cancelled}</div>
            </CardContent>
          </Card>
          <Card className="bg-white border-slate-200">
            <CardContent className="pt-6">
              <div className="text-3xl font-bold text-slate-600">{ordersLoading ? '—' : stats.deleted}</div>
            ...
            .map((module) =>
              module.disabled ? (
                ...
                >
                  <CardHeader>
                    <div
                      className={`w-12 h-12 ${module.color} rounded-lg flex items-center justify-center mb-3 opacity-50`}
                    >
            ...
            {(() => {
              ...
              return (
                <Link to={createPageUrl('ClientKYC')}>
                  <div
                    className={`relative rounded-xl p-6 border transition-all hover:shadow-md group h-full ${needsAttention ? 'bg-red-50 border-red-300' : 'bg-white border-slate-200 hover:border-slate-300'}`}
                  >
                    {needsAttention && (
                    ...
                    <div className="flex items-start gap-4">
                      <div
                        className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 transition-colors ${needsAttention ? 'bg-red-100 group-hover:bg-red-200' : 'bg-slate-100 group-hover:bg-slate-200'}`}
                      >
