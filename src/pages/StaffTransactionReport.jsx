...
const TYPE_BADGE = { ingoing: 'bg-blue-100 text-blue-700', outgoing: 'bg-orange-100 text-orange-700', domestic: 'bg-slate-100 text-slate-700' };
const RISK_BADGE = { low: 'bg-green-100 text-green-700', medium: 'bg-yellow-100 text-yellow-700', high: 'bg-red-100 text-red-700' };
...
export default function StaffTransactionReport() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries(['transaction-reports']); toast.success('Record deleted'); }
  });
  ...
  const filtered = useMemo(() => {
    ...
    if (search) {
      ...
      r = r.filter(x => [x.transaction_id, x.sender_name, x.sender_id, x.recipient_name, x.currency]
  ...
  const clearFilters = () => {
    setSearch(''); setFilterType('all'); setFilterCurrency('all');
    setFilterRisk('all'); setFilterDTTOT('all'); setFilterDPPPSPM('all');
    setCurrentPage(1);
  };

  const openEdit = (row) => { setEditingRecord(row); setShowModal(true); };
  const openAdd = () => { setEditingRecord(null); setShowModal(true); };
  const onSaved = () => { setShowModal(false); queryClient.invalidateQueries(['transaction-reports']); };
      ...
      <header className="bg-[#1e3a5f] shadow-lg">
        <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-lg font-bold text-white">Transaction Data Report</h1>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          <div className="flex flex-wrap gap-3 items-center">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <Input
                placeholder="Search ID, sender, recipient, currency…"
                value={search}
            ...
            <Select value={filterDTTOT} onValueChange={v => { setFilterDTTOT(v); setCurrentPage(1); }}>
              <SelectTrigger className="w-36"><SelectValue placeholder="DTTOT" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="all">DTTOT: All</SelectItem>
                <SelectItem value="true">DTTOT: Yes</SelectItem>
                <SelectItem value="false">DTTOT: No</SelectItem>
              </SelectContent>
            </Select>
            <Select value={filterDPPPSPM} onValueChange={v => { setFilterDPPPSPM(v); setCurrentPage(1); }}>
              <SelectTrigger className="w-40"><SelectValue placeholder="DPPPSPM" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="all">DPPPSPM: All</SelectItem>
                <SelectItem value="true">DPPPSPM: Yes</SelectItem>
                <SelectItem value="false">DPPPSPM: No</SelectItem>
              </SelectContent>
            </Select>
              ...
              <tbody className="divide-y divide-slate-100">
                {isLoading ? (
                  ...
                  <tr key={row.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-3 py-2.5 font-mono text-xs text-slate-700 whitespace-nowrap">{row.transaction_id}</td>
                    ...
                    <td className="px-3 py-2.5 text-xs font-medium text-slate-800 whitespace-nowrap">{row.account_holder_name}</td>
                    <td className="px-3 py-2.5 font-mono text-xs text-slate-600 whitespace-nowrap">{row.account_number}</td>
                    ...
                    <td className="px-3 py-2.5 text-xs text-slate-500 max-w-[120px] truncate" title={row.recipient_address}>{row.recipient_address}</td>
                    <td className="px-3 py-2.5 text-xs text-right font-medium text-slate-700 whitespace-nowrap">{row.transfer_fee != null ? row.transfer_fee.toLocaleString() : ''}</td>
                    <td className="px-3 py-2.5 whitespace-nowrap">
                      <span className="px-1.5 py-0.5 rounded text-xs bg-slate-100 text-slate-700">{row.beneficiary_type}</span>
                    </td>
                    <td className="px-3 py-2.5 whitespace-nowrap">
                      <span className={`px-1.5 py-0.5 rounded text-xs font-medium capitalize ${RISK_BADGE[row.risk_level] || 'bg-slate-100 text-slate-600'}`}>{row.risk_level}</span>
                    </td>
                    <td className="px-3 py-2.5 text-center">
                      <span className={`px-1.5 py-0.5 rounded text-xs font-bold ${row.dttot_check ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>{row.dttot_check ? 'Yes' : 'No'}</span>
                    </td>
                    <td className="px-3 py-2.5 text-center">
                      <span className={`px-1.5 py-0.5 rounded text-xs font-bold ${row.dpppspm_check ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>{row.dpppspm_check ? 'Yes' : 'No'}</span>
                    </td>
                    <td className="px-3 py-2.5">
                      <div className="flex gap-1">
                        <Button size="icon" variant="ghost" className="h-6 w-6" onClick={() => openEdit(row)}><Edit2 className="w-3 h-3" /></Button>
                        <Button size="icon" variant="ghost" className="h-6 w-6 text-red-500 hover:bg-red-50" onClick={() => { if (window.confirm('Delete this record?')) deleteMutation.mutate(row.id); }}><Trash2 className="w-3 h-3" /></Button>
                      </div>
                    </td>
                  </tr>
                ))}
