...
export default function StaffCustomerReport() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries(['customer-reports']); toast.success('Record deleted'); }
  });
  ...
  const filtered = useMemo(() => {
    ...
    if (search) {
      ...
      r = r.filter(x => [x.name, x.legal_tax_number, x.director_name, x.address, x.id]
  ...
  const clearFilters = () => { setSearch(''); setFilterType('all'); setFilterCode('all'); setFilterPEP('all'); setFilterCitizen('all'); setCurrentPage(1); };
  const openEdit = (row) => { setEditingRecord(row); setShowModal(true); };
  const openAdd = () => { setEditingRecord(null); setShowModal(true); };
  const onSaved = () => { setShowModal(false); queryClient.invalidateQueries(['customer-reports']); };
              ...
              <tbody className="divide-y divide-slate-100">
                {isLoading ? (
                  ...
                  <tr key={row.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-3 py-2.5 text-xs text-slate-600 whitespace-nowrap">{row.registration_number}</td>
                    ...
                    <td className="px-3 py-2.5 whitespace-nowrap">
                      <span className="px-1.5 py-0.5 rounded text-xs bg-slate-100 text-slate-700">{row.code_type}</span>
                    </td>
