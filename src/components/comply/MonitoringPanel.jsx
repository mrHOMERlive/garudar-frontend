...
export default function MonitoringPanel({ onSelectCustomer }) {
  ...
  const filtered = (data || []).filter(
    (c) => !search || c.name?.toLowerCase().includes(search.toLowerCase()) || c.external_identifier?.includes(search)
  );
    ...
    <div className="space-y-4">
      ...
      {loading ? (
        <div className="flex items-center gap-2 text-slate-500 py-8 justify-center">
          <Loader2 className="w-5 h-5 animate-spin" /> Loading monitored customers...
        </div>
      ) : filtered.length === 0 ? (
        ...
        <div className="space-y-2">
          <p className="text-sm text-slate-500">
            {filtered.length} monitored customer{filtered.length !== 1 ? 's' : ''}
          </p>
                ...
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      onClick={(e) => handleDisable(customer.id, e)}
                      disabled={disabling[customer.id]}
                    >
                      {disabling[customer.id] ? (
